...
object ParserTest extends App {
  ...
  def extractClasses(filename: String): List[TsClass] = {
    ...
    content.split('\n').foreach {
      ...
      case fieldTs(name, type1) =>
        if (name.endsWith("?")) {
          if (type1.endsWith("[]"))
            members += TsMember(cookName(name.dropRight(1)), s"List[${cookType(type1.dropRight(2))}] = Nil")
          else
            members += TsMember(cookName(name.dropRight(1)), s"Option[${cookType(type1)}] = None")
        } else {
          if (type1.endsWith("[]"))
            members += TsMember(cookName(name), s"List[${cookType(type1.dropRight(2))}]")
          else
            members += TsMember(cookName(name), s"${cookType(type1)}", true)
        }
      case s => s
    }
    currentClass.map { c =>
  ...
  val classes = extractClasses(
    "C:\\Projects\\it-gov\\elasticsearch-specification\\specification\\ingest\\_types\\Processors.ts",
  )
