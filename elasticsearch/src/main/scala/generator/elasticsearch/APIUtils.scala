...
object APIUtils {
  ...
  def convertToScalaDefault(
    ...
    if (default.isDefined) {
      val value = default.get.toString()
      return Some(value)
    }
  ...
  def saveFile(filename: os.Path, content: String): Unit = {
    os.write(filename, content, createFolders = true)

    println(s"Generated ${filename.toString}")
