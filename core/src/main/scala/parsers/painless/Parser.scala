...
object PainlessParser {
  ...
  def number[_: P] = P(CharIn("+\\-").? ~ integral ~ fractional.? ~ exponent.?).!.map(x => Painless.Num(x.toDouble))
  ...
  def accessTypeMethod[_: P] =
    P(accessTypeSimple ~/ "(" ~/ space ~ painlessExpr ~/ space ~ ")").map(v =>
      Painless.AccessTypeWithMethod(v._1, v._2),
    )
